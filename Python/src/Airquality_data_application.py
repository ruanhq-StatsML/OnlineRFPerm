...
MODEL_REGISTRY = model_factory.as_r_style_dict()
df = pd.read_csv("Real_Data/Air_Quality_Data/AirQualityUCI.csv", delimiter = ';')
df = df[df["Date"].notna()]
FEATURE_COLS = ['PT08.S1(CO)', 'PT08.S2(NMHC)', 'PT08.S3(NOx)', 'PT08.S4(NO2)', 'PT08.S5(O3)',
'T', 'RH', 'AH']
TARGET_COL = ["NO2(GT)"]
...
for cols in FEATURE_COLS:
    if df[cols].dtype == 'O':
        df_X[cols] = df_X[cols].apply(lambda X: X.replace(',', '.'))
        df_X[cols] = df_X[cols].astype(float)
df_whole = pd.concat([df_X, df_Y], axis = 1)
...
pd.DataFrame(result, columns = cols).to_csv("airquality_onlinePermOOB_result.csv")
