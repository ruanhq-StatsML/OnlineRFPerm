#NYC Taxi Data Evaluation:
#Evaluation on Interstate loading:
...
daily_list = Path("Real_Data/NYC_Taxi/Daily_Slice")
files = sorted(daily_list.glob('*pickup_df.csv'))
X_cols = ["PULocationID", "DOLocationID", "passenger_count", "trip_distance", "RatecodeID", "payment_type"]
TARGET_COL = 'total_amount'
df['pickup_dt'] = pd.to_datetime(df['tpep_pickup_datetime'])
cols_result = ['BOCPD_SUM', 'BOCPD_bcpd_2', 'BOCPD_bcpd_1', 'BOCPD_bcpd_3', 'EWMA_SUM', 'EWMA_3', 'EWMA_2',
'EWMA_1', 'TS_SUM', 'TS_3', 'TS_2', 'TS_1', 'ADWIN_SUM', 'ADWIN_3', 'ADWIN_2', 'ADWIN_1', 'HDMMA_SUM', 
'HDMMA_3', 'HDMMA_2', 'HDMMA_1', 'ECDDWT_SUM', 'ECDDWT_3', 'ECDDWT_2', 'HalfSpaceTrees_SUM', 'HalfSpaceTrees_1',
'HalfSpaceTrees_2', 'HalfSpaceTrees_3', 'martingale_SUM', 'martingale2_SUM', 'martingale_1', 'martingale_2', 
'martingale_3', 'martingale2_1', 'martingale2_2', 'martingale2_3', 'fix_SUM', 'fix_1', 'fix_2', 'fix_3',
'addis_SUM', 'addis_1', 'addis_2', 'addis_3', 'saffron_SUM', 'saffron_1', 'saffron_2', 'saffron_3', 
'DDM_SUM', 'DDM_1', 'DDM_2', 'DDM_3', 'STEPD_SUM', 'STEPD_1', 'STEPD_2', 'STEPD_3', 'HDDMA_SUM', 
'HDDMA_1', 'HDDMA_2', 'HDDMA_3', 'ECDDWT_1', 'name', 'pickup']
for file in files:
    dfs = pd.read_csv(str(file)).drop(['Unnamed: 0'], axis = 1)
    if dfs.shape[0] <= 5000:
        continue
    X_cols = ["PULocationID", "DOLocationID", "passenger_count", "trip_distance", "RatecodeID", "payment_type"]
    dfs['pickup_dt'] = pd.to_datetime(dfs['tpep_pickup_datetime'])
    for c in X_cols:
        if dfs[c].isna().any():
            dfs[c] = dfs[c].fillna(dfs[c].median())
    dfs = dfs.sort_values(['pickup_dt'])
    dfs['pickup_dt'] = pd.to_datetime(dfs['tpep_pickup_datetime'])
    dfs['pickup_hour'] = dfs['pickup_dt'].dt.strftime('%Y-%m-%d %H')
    dfs['pickup_date'] = dfs['pickup_dt'].dt.date
    if dfs['pickup_dt'][0] <= datetime.strptime('2023-01-09', '%Y-%m-%d'):
        continue
    dfs['pickup_hour'] = dfs['pickup_dt'].dt.hour
    df_date = str(dfs['pickup_date'][0])
    dfs['minute_idx'] = dfs['pickup_dt'].dt.hour * 60 + dfs['pickup_dt'].dt.minute
    n_ref5 = max(dfs[dfs['pickup_hour'] == 5].shape[0], 100)
    n_ref7 = max(dfs[dfs['pickup_hour'] == 7].shape[0], 100)
    n_ref11 = max(dfs[dfs['pickup_hour'] == 11].shape[0], 100)
    n_ref15 = max(dfs[dfs['pickup_hour'] == 15].shape[0], 100)
    df_X = np.array(dfs[X_cols])
    df_Y = np.array(dfs[TARGET_COL]).reshape(-1, 1)
    df = np.hstack([df_X, df_Y])
    #5-6 am
    result = []
    df5 = dfs[dfs['pickup_hour'] >= 5]
    df_X = np.array(df5[X_cols])
    df_Y = np.array(df5[TARGET_COL]).reshape(-1, 1)
    df = np.hstack([df_X, df_Y])
    df = df[:min(8000, df.shape[0]), :]
    result_xgb1 = onlinePermOOB_wholedf(df,
        model_registry = MODEL_REGISTRY,
        model_m = 'xgb_regressor',
        ref_batch_size = n_ref5, 
        batch_size = 1, 
        seed = 2026
        )
    result_xgb1['name'] = 'xgb'
    result_xgb1['pickup'] = 5  
    result.append(result_xgb1)
    result_rf1 = onlinePermOOB_wholedf(df,
        model_registry = MODEL_REGISTRY,
        model_m =  'rf_regressor',
        ref_batch_size = n_ref5, 
        batch_size = 1, 
    ...
    result_rf1['name'] = 'rf'
    result_rf1['pickup'] = 5     
    result.append(result_rf1)
    result_mlp1 = onlinePermOOB_wholedf(df,
        model_registry = MODEL_REGISTRY,
    ...
    result_mlp1['name'] = 'mlp'
    result_mlp1['pickup'] = 5         
    result.append(result_mlp1)
    #pd.DataFrame(result, columns = cols_result).to_csv(f"Real_Data/NYC_Taxi/Daily_results/NYCTaxi_{df_date}_onlinePermOOB_result.csv")
    pd.DataFrame(result, columns = cols_result).to_csv(f"Real_Data/NYC_Taxi/Daily_results/NYCTaxi_{df_date}_onlinePermOOB_result.csv")    
    #7-8 am
    df7 = dfs[dfs['pickup_hour'] >= 7]
    df_X = np.array(df7[X_cols])
    df_Y = np.array(df7[TARGET_COL]).reshape(-1, 1)
    df = np.hstack([df_X, df_Y])
    df = df[:min(8000, df.shape[0]), :]
    result_xgb1 = onlinePermOOB_wholedf(df,
        model_registry = MODEL_REGISTRY,
        model_m = 'xgb_regressor',
        ref_batch_size = n_ref7, 
        batch_size = 1, 
        seed = 2026
        )
    result_xgb1['name'] = 'xgb'
    result_xgb1['pickup'] = 7
    result.append(result_xgb1)
    result_rf1 = onlinePermOOB_wholedf(df,
        model_registry = MODEL_REGISTRY,
        model_m =  'rf_regressor',
    ...
    result_mlp1['name'] = 'mlp'
    result_mlp1['pickup'] = 7
    result.append(result_mlp1)
    pd.DataFrame(result, columns = cols_result).to_csv(f"Real_Data/NYC_Taxi/Daily_results/NYCTaxi_{df_date}_onlinePermOOB_result.csv")    
    #11-12 am
    df11 = dfs[dfs['pickup_hour'] >= 11]
    df_X = np.array(df11[X_cols])
    df_Y = np.array(df11[TARGET_COL]).reshape(-1, 1)
    df = np.hstack([df_X, df_Y])
    df = df[:min(8000, df.shape[0]), :]
    result_xgb1 = onlinePermOOB_wholedf(df,
        model_registry = MODEL_REGISTRY,
        model_m = 'xgb_regressor',
        ref_batch_size = n_ref11, 
        batch_size = 1, 
        seed = 2026
        )
    result_xgb1['name'] = 'xgb'
    result_xgb1['pickup'] = 11
    result.append(result_xgb1)
    result_rf1 = onlinePermOOB_wholedf(df,
        model_registry = MODEL_REGISTRY,
        model_m =  'rf_regressor',
    ...
    result_mlp1['name'] = 'mlp'
    result_mlp1['pickup'] = 11
    result.append(result_mlp1)
    pd.DataFrame(result, columns = cols_result).to_csv(f"Real_Data/NYC_Taxi/Daily_results/NYCTaxi_{df_date}_onlinePermOOB_result.csv")
    #3-4 pm
    df15 = dfs[dfs['pickup_hour'] >= 11]
    df_X = np.array(df15[X_cols])
    df_Y = np.array(df15[TARGET_COL]).reshape(-1, 1)
    df = np.hstack([df_X, df_Y])
    df = df[:min(8000, df.shape[0]), :]
    result_xgb1 = onlinePermOOB_wholedf(df,
        model_registry = MODEL_REGISTRY,
        model_m = 'xgb_regressor',
        ref_batch_size = n_ref15, 
        batch_size = 1, 
        seed = 2026
        )
    result_xgb1['name'] = 'xgb'
    result_xgb1['pickup'] = 15
    result.append(result_xgb1)
    result_rf1 = onlinePermOOB_wholedf(df,
        model_registry = MODEL_REGISTRY,
        model_m =  'rf_regressor',
