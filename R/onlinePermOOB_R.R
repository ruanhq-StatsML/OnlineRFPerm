...
############################################################
#' Online PermOOB procedure - under the regression setting
#'
...
onlinePermOOB_regression_prototype <- function(data, model_spec,
  ...
  results <- map_dfr(batch_ids, function(b, QA = FALSE){
    ...
    if(QA){
      ...
      mse_val = (preds$.pred - y) ** 2
    }
    #mse_val <- preds %>% rmse(truth = y, estimate = .pred) %>%
    #pull(.estimate) %>% {.^2}
...
'''
- Did not reject anything here via the OnlineFDR procedure but for the 
